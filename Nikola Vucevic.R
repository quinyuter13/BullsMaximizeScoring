...
## How can we maximize Nikola Vucevic's points?
...
shot_locations_21_22 <- read_csv("Vucevic_location_21_22.csv")
shot_locations_22_23 <- read_csv("Vucevic_location_22_23.csv")
shot_types_21_22 <- read_csv("Vucevic_types_21_22.csv")
shot_types_22_23 <- read_csv("Vucevic_types_22_23.csv")
...
## Not including back court, as Vucevic only took two shots from this location, 
##    and missed both. Further, not including mid range 3 point shot, as he
##    took one shot from this location, and missed.

points_percentage_vector_location <- c(locations_final[1, 6]*2, locations_final[2, 6]*2, 
                                       (134/319)*2, locations_final[4, 7]*3, 
                                       locations_final[5, 7]*3, locations_final[6, 7]*3)

## Simulating if Vucevic would take 1000 attempts at each shot, 
...
total_area_points <- data.frame(lapply(points_percentage_vector_location,"*", 1000))%>%
  rename('Restricted Area' = percent_FG, "In the Paint" = percent_FG.1, 
         "Mid Range" = X840.12539184953, "Left Corner 3" = percent_3P, 
         "Right Corner 3" = percent_3P.1, "Above Break 3" = percent_3P.2)

##################################################################

## Creating a vector of each of the percentages of each shot
##    type that Vucevic takes.
## Each index included in the vector is only the percent of each
##    shot type, so [1, 6] is Alley Oop percentage, [3, 6]
##    is Dunk Shot, and so on.
...
## Did not include 3 point bank Shot or fadeaway, as there was not a high enough 
##    sample to get an accurate response.

points_percentage_vector_type <- c(types_final[1, 6]*2, (16/23)*2, 
                                   types_final[3, 6],
                                   (12/26)*2, types_final[5, 6]*2, 
                                   (160/314)*2, types_final[6, 7]*3, 
                                   (232/470)*2, types_final[7, 7]*3,
                                   types_final[8, 6]*2, types_final[9, 6]*2)
...
total_types_points <- data.frame(lapply(points_percentage_vector_type,"*", 1000)) %>%
  rename('Alley Oop' = percent_FG, 'Bank Shot' = X1391.30434782609, 
         'Dunk' = percent_FG.1, 'Fadeaway' = X923.076923076923, 
         'Finger Roll' = percent_FG.2, 'Hook Shot 2pt' = X1019.10828025478, 
         'Hook Shot 3pt' = percent_3P, 'Jump Shot 2pt' = X987.234042553191, 
         'Jump Shot 3pt' = percent_3P.1, 'Layup' = percent_FG.3, 
         'Tip Shot' = percent_FG.4)

#################################################################################
## Conclusion

## Per 1000 attempts, Vucevic scored the most points in the restricted
##    area, for a total of 1367.713 points. The second most points he scored
##    was from the right corner 3 point line, for a total of 1350 points.

## Per 1000 attempts, Vucevic scored the most points taking an Alley Oop shot for a 
##    total of 1428.571 points. In terms of shots that can be taken from
##    three point line, Hook shots yield the highest amount of points for Vucevic,
##    where he would shoot a total of 1139.037 points.

## So, in order for Nikola Vucevic to score the most amount of points possible,
##    he should either shoot an Alley Oop from the restricted area,
##    or a Hook shot from either the right corner 3 point line.         
